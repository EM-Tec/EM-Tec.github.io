...
HTML就是這樣建立一個又一個地”元素” (element) 。為了方便操作我們把輸入框放到左邊，然後選一個好看的顏色，讓我們一次認識所有常見HTML吧
...
`<p>`元素代表段落區塊(paragraph)。`<b>`是粗體(bold)、`<i>`是斜體(Italic)、`<s>`是劃掉(Strike)、`<u>`是底線(Underline)。`<sup>`是上標(Superscript)、`<sub>`是下標(Subscript)。實際效果如下:
...
假設你想要連結到某個網站，你可以使用`<a>`元素，並在`href`屬性中指定連結的網址，而在`<a>`元素中間的文字就是連結的文字。
